...
// ফি সংগ্রহ পেজ - স্টুডেন্ট ড্রপডাউন ফিক্স + লেনদেন হিস্ট্রি
...
$transactions = [];
if ($selected_student_id) {
    $stmt = $pdo->prepare("SELECT fp.*, ft.name_bn as fee_type_name 
...
<div class="container-fluid py-3">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div>
            <a href="pending.php" class="btn btn-danger btn-sm me-2"><i class="fas fa-exclamation-circle"></i> বাকিয়া</a>
            <a href="reports.php" class="btn btn-outline-secondary btn-sm"><i class="fas fa-chart-bar"></i> রিপোর্ট</a>
        </div>
    </div>
            ...
            <div class="row g-2 align-items-end">
                ...
                <div class="col-md-2">
                    ...
                    <select id="class_filter" class="form-select form-select-sm" onchange="loadStudentsByClass()">
                        <option value="">প্রথম শ্রেণী</option>
    ...
    <!-- মেইন কন্টেন্ট: বাম=ফি ফর্ম, ডান=হিস্ট্রি -->
    <div class="row">
        <!-- বাম পাশ: ফি জমা দিন -->
        <div class="col-lg-7">
            <div class="card">
                        ...
                        <form method="POST" action="process_payment.php" id="feeForm">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="fee_type_id" class="form-select" required onchange="updateAmount(this)">
                                    ...
                                    <?php
                                    $fee_types = $pdo->query("SELECT * FROM fee_types WHERE status = 'active' ORDER BY name_bn")->fetchAll();
                                    foreach ($fee_types as $ft):
                                    ?>
                                        <option value="<?= $ft['id'] ?>" data-amount="<?= $ft['amount'] ?>">
                                            <?= htmlspecialchars($ft['name_bn']) ?> (<?= $ft['amount'] ?> ৳)
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="row mb-3">
                                <div class="col-6">
                                    ...
                                    <select name="payment_method" class="form-select">
                                        ...
                                        <option value="নগদ_মোবাইল">নগদ (মোবাইল)</option>
        ...
        <!-- ডান পাশ: পূর্ববর্তী পরিশোধ / লেনদেন হিস্ট্রি -->
        <div class="col-lg-5">
            <div class="card">
                <div class="card-header bg-white">
                    <h6 class="mb-0"><i class="fas fa-history"></i> পূর্ববর্তী পরিশোধ</h6>
                </div>
                <div class="card-body p-0" id="transaction_history">
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td class="small text-end fw-bold"><?= number_format($tr['amount']) ?> ৳</td>
                                            <td class="small"><?= htmlspecialchars($tr['payment_method'] ?? 'নগদ') ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
            ...
            <div class="card mt-3">
                <div class="card-header bg-white">
                    <h6 class="mb-0 text-danger"><i class="fas fa-exclamation-triangle"></i> বাকিয়া সামারি</h6>
                </div>
                <div class="card-body" id="dues_summary">
                    <div class="text-center text-muted py-2">
                        <small>লোড হচ্ছে...</small>
                    </div>
                </div>
            </div>
            <?php endif; ?>
...
function loadStudentsByClass() {
    ...
    if (!classId) {
        studentSelect.innerHTML = '<option value="">-- ছাত্র নির্বাচন করুন --</option>';
        return;
    }
    ...
    .then(data => {
        ...
        data.forEach(s => {
            html += '<option value="' + s.id + '">' + s.name_bn + ' - রোল: ' + (s.roll_number || '-') + ' (' + s.student_id + ')</option>';
        });
        studentSelect.innerHTML = html;
    })
    .catch(() => {
        studentSelect.innerHTML = '<option value="">লোড ব্যর্থ</option>';
    });
}

// ছাত্র সিলেক্ট করলে পেজ রিলোড
        ...
        .then(data => {
            if (data.length === 0) {
                ...
                data.forEach(s => {
                    ...
                    html += '<strong>' + s.name_bn + '</strong><br>';
                    html += '<small class="text-muted">ID: ' + s.student_id + ' • ' + (s.class_name || '') + ' • রোল: ' + (s.roll_number || '-');
                    if (s.division_name) html += ' • ' + s.division_name;
                    html += '</small></a>';
                });
...
// বাকিয়া সামারি লোড
<?php if ($student): ?>
fetch('../../api/get_dues.php?student_id=<?= $student['id'] ?>')
.then(r => r.json())
.then(data => {
    const div = document.getElementById('dues_summary');
    if (data.total_due > 0) {
        div.innerHTML = '<div class="d-flex justify-content-between"><span>মোট বাকিয়া:</span><strong class="text-danger">' + data.total_due + ' ৳</strong></div>';
    } else {
        div.innerHTML = '<div class="text-center text-success"><i class="fas fa-check-circle"></i> কোনো বাকিয়া নেই</div>';
    }
}).catch(() => {
    document.getElementById('dues_summary').innerHTML = '<small class="text-muted">লোড ব্যর্থ</small>';
});
<?php endif; ?>

// পেজ লোডে বিভাগ ফিল্টার
document.addEventListener('DOMContentLoaded', function() {
    filterClasses();
    // যদি class_id URL-এ থাকে তাহলে ছাত্র লোড
    const classFilter = document.getElementById('class_filter');
    if (classFilter.value) loadStudentsByClass();
});

// বাইরে ক্লিক করলে সার্চ রেজাল্ট বন্ধ
