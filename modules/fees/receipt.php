...
$stmt = $db->prepare("SELECT fc.*, ft.fee_name_bn, s.name_bn as sname, s.name as sname_en,
    s.student_id as sid, s.father_name, c.class_name_bn
    FROM fee_collections fc
    JOIN fee_types ft ON fc.fee_type_id=ft.id
    JOIN students s ON fc.student_id=s.id
    LEFT JOIN classes c ON s.class_id=c.id
    WHERE fc.id=?");
$stmt->execute([$id]); $rec = $stmt->fetch();
...
<div class="card" style="max-width:580px;margin:0 auto;">
    <div style="padding:24px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:14px;margin-bottom:16px;">
            ...
            <div><span style="color:var(--text-muted);"><?=e($k)?>: </span><strong><?=e($v)?></strong></div>
            <?php endforeach; ?>
        </div>
        <!-- Amount -->
        <div style="background:var(--bg);border-radius:8px;padding:16px;margin:16px 0;">
            <div style="display:flex;justify-content:space-between;padding:4px 0;font-size:13px;"><span>মূল পরিমাণ</span><span>৳<?=number_format($rec['amount'])?></span></div>
            <?php if($rec['discount']>0): ?>
            <div style="display:flex;justify-content:space-between;padding:4px 0;font-size:13px;color:var(--success);"><span>ছাড়</span><span>-৳<?=number_format($rec['discount'])?></span></div>
            <?php endif; if($rec['fine']>0): ?>
            <div style="display:flex;justify-content:space-between;padding:4px 0;font-size:13px;color:var(--danger);"><span>জরিমানা</span><span>+৳<?=number_format($rec['fine'])?></span></div>
            <?php endif; ?>
            <div style="display:flex;justify-content:space-between;padding:8px 0 0;font-size:18px;font-weight:700;border-top:1px solid var(--border);margin-top:8px;">
                ...
                <span style="color:var(--success);">৳<?=number_format($rec['paid_amount'])?></span>
            </div>
        </div>
