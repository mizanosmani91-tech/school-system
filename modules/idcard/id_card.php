...
if ($type === 'teacher') {
    ...
    if ($filterIds) {
        $ids = array_map('intval', explode(',', $filterIds));
        $ph  = implode(',', array_fill(0, count($ids), '?'));
        $st  = $db->prepare("SELECT s.*,c.class_name_bn,c.class_name,sec.section_name FROM students s LEFT JOIN classes c ON s.class_id=c.id LEFT JOIN sections sec ON s.section_id=sec.id WHERE s.id IN($ph) AND s.status='active' ORDER BY s.roll_number");
        $st->execute($ids); $people = $st->fetchAll();
    } elseif ($filterClass) {
        $st = $db->prepare("SELECT s.*,c.class_name_bn,c.class_name,sec.section_name FROM students s LEFT JOIN classes c ON s.class_id=c.id LEFT JOIN sections sec ON s.section_id=sec.id WHERE s.class_id=? AND s.status='active' ORDER BY s.roll_number");
...
<div class="card mb-16 no-print">
    <div class="card-body" style="padding:16px 20px;">
        ...
        <?php if (!empty($people) && (($type==='student'&&$filterClass)||$type!=='student')): ?>
...
<div class="card no-print"><div class="card-body" style="text-align:center;padding:48px;color:var(--text-muted);">
    ...
    <p style="font-size:16px;"><?= $type==='student'?'শ্রেণী নির্বাচন করুন':'কোনো তথ্য পাওয়া যায়নি' ?></p>
</div></div>
<?php endif; ?>
...
/* প্রতিটি পেয়ার: Back + Front পাশাপাশি */
.idc-pair {
    display: flex;
    flex-direction: row;
    flex-wrap: nowrap;
...
.idc-ph-x {
    width:80px; height:95px; background:#f0f8f0;
    border:3px solid #e67e22; border-radius:4px;
...
.idc-id {
    ...
    color:#555; margin:2px 0 4px; letter-spacing:.5px; flex-shrink:0;
}
...
.idc-wm {
    ...
    font-size:86px; color:rgba(26,138,60,.05);
...
@media print {
    ...
    #idc-container {
        ...
        gap:6mm !important; padding:5mm !important; margin:0 !important;
    }
...
function onTypeChange(sel){
    ...
    if(cd) cd.style.display=sel.value==='student'?'':'none';
